...
our @ISA    = qw(Exporter);
our @EXPORT = qw( get_sf_token Get_SF_Posventas Get_Xena_Posventas ADD_Alineamiento ADD_MSISDN_To_Alineamiento UPDATE_Num_Msisdn_Alineamiento);
...
sub ADD_Alineamiento {
	...
	$sql = "INSERT INTO topercable.alineamiento_planificado (TIPO, NOMBRE, ESTADO, num_msisdn, creador, prioridad) VALUES ('Posventas', Concat(Date_Sub(Curdate(), interval 1 DAY),' - ', DATE_FORMAT(Date_Sub(Curdate(), interval 1 day), '%W'),' -  Posventas (Tarifas, Bonos, SIM)'),0,'$num_msisdn','ToperCable','2') ";
	$sthd = $dbhd->prepare($sql);	
	$sthd->execute() or die ("No se pudo ejecutar la consulta. SQL:($sql)  Desc(" . $sthd->errstr . ")");
...
sub Get_Xena_Posventas {
	#	Funcion que obtiene los MSISDN de Posventas (Cambios de estado / Pasarela Cablemovil) directamente desde Xena
	my $nombre_modulo = (caller(0))[3];
	Plogged($log_file, $modo_ejecucion, 0, "\t -> $nombre_modulo -> Inicio ");
...
FROM (
    SELECT p.msisdn
    FROM mvno.pv_msisdn p
    LEFT JOIN mvno.pv_msisdn a
           ON a.msisdn = p.msisdn
          AND a.Fecha_activacion = DATE_FORMAT(CURDATE() - INTERVAL 1 DAY, '%Y%m%d')
    WHERE p.Fecha_Estado_Actual = DATE_FORMAT(CURDATE() - INTERVAL 1 DAY, '%Y%m%d')
      AND a.msisdn IS NULL
    LIMIT 30000
    UNION
    SELECT p.msisdn
    FROM mnet.pv_pasarela_cablemovil p
    LEFT JOIN mvno.pv_msisdn a
           ON a.msisdn = p.msisdn
          AND a.Fecha_activacion = DATE_FORMAT(CURDATE() - INTERVAL 1 DAY, '%Y%m%d')
    WHERE p.fecha_solicitud = DATE_FORMAT(CURDATE() - INTERVAL 1 DAY, '%Y%m%d')
      AND a.msisdn IS NULL
    LIMIT 30000
) t
LIMIT 25000";
	...
	unless ($sthd->execute()) {
		...
		return { result => 0, Xena_Response => "Error SQL pv_msisdn/pv_pasarela_cablemovil: " . $sthd->errstr };
	}

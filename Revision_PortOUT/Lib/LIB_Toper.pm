# LIB_Toper.pm
#
# Libreria con las funciones locales
...
our @ISA    = qw(Exporter);
our @EXPORT = qw( get_sf_token Get_SF_PortOUT ADD_Alineamiento ADD_MSISDN_To_Alineamiento);
...
sub ADD_Alineamiento {
	...
	$sql = "INSERT INTO topercable.alineamiento_planificado (TIPO, NOMBRE, ESTADO, num_msisdn, creador, prioridad) VALUES ('PortOUT', Concat(Date_Sub(Curdate(), interval 1 DAY),' - ', DATE_FORMAT(Date_Sub(Curdate(), interval 1 day), '%W'),' -  Port_OUT'),0,'$num_msisdn','ToperCable','3') ";
	$sthd = $dbhd->prepare($sql);	
	$sthd->execute() or die ("No se pudo ejecutar la consulta. SQL:($sql)  Desc(" . $sthd->errstr . ")");
...
sub ADD_MSISDN_To_Alineamiento {
    ...
    Plogged($log_file, $modo_ejecucion,0,"\t -> msisdn: " . ($msisdn // 'undef'));
...
sub Get_SF_PortOUT {
    ...
    #my $sql_query = "SELECT RM_MSISDN_Portabilidad__c FROM RM_Solicitud_Portabilidad__c WHERE CreatedDate = yesterday and RM_Estado_Solicitud__c = 'ERROR' limit 20000";
	# 2026-01-02 Añadimos PortOUT en AHOL (Estado temporal que deberia haber pasado a un estado terminado)
	my $sql_query = "SELECT RM_MSISDN_Portabilidad__c FROM RM_Solicitud_Portabilidad__c WHERE CreatedDate = LAST_N_DAYS:3 and RM_Estado_Solicitud__c in ('ERROR','AHOL') and RM_Fecha_Ejecucion_Portabilidad__c < today and Activo__r.Status != 'Deleted' and RM_Orden_Baja__r.MM_Estado_BPM__c != 'Cancelled' limit 10000";
    ...
    # Eliminar duplicados por RM_MSISDN_Portabilidad__c
    my %visto;
    my @unicos = grep {
        my $tel = $_->{RM_MSISDN_Portabilidad__c} // '';
        !$visto{$tel}++
    } @all_results;

...
my $dir_principal			= "D:\\Intranet\\Script\\ToperCable\\Alineamientos_Planificados_Inventario_Run\\LOGS\\";
...
Plogged ($log_file,$modo_ejecucion,1,"- Creamos Directorio LOGS para la ejecucion WGet");
eval {
    make_path($directorio_log, { mode => 0755 });
};
	if ($@) {
		my $error_message = $@;
		Plogged($log_file, $modo_ejecucion, 1, "ERROR. Creacion del directorio Fallida: $error_message");
		my $texto_asunto = "ERROR. La creacion del directorio LOG fue fallida.";
		my $texto_mail = "<b><font color=red>Error del JOB Alineamientos Planificados Cablemovil</b><br><br>No se pudo crear el directorio de LOGS.</b></font>";
		...
		Plogged($log_file, $modo_ejecucion, 3, "ERROR. Creacion del directorio Fallida: $error_message");
		exit 1;
	} else {
...
# Plogged ($log_file,$modo_ejecucion,1,"- Creamos el Directorio LOG para la ejecucion del WGet");
	# $salida_system = system("mkdir $directorio_log ");	
		# if ($salida_system == 0){
			# Plogged ($log_file,$modo_ejecucion,0,"OK. Creacion del directorio correcta");
			
		# }else{					
			# Plogged ($log_file,$modo_ejecucion,1,"ERROR. Creacion del directorio Fallida ($salida_system).");						
			# $texto_asunto = "ERROR. La creacion del directorio LOG fue fallida.";
			# $texto_mail = "<b><font color=red>Mensaje de error del JOB Alineamientos Planificados Inventario BPM</b><br><br>No se pudo crear el directorio para los LOGS del envio.</b></font>";
			# $texto_mail = $texto_mail . "<br><br><font color=black>ToperCable NO puede continuar con el proceso.<br><br><b>ToperCable :.<br>MASMOVIL</b></font>";

			# Plogged_Mail ($email_sistemas,$log_file,$texto_asunto,$texto_mail);

			# Plogged ($log_file,$modo_ejecucion,3,"ERROR. Creacion del directorio Fallida ($salida_system).");
			# exit 1;
		# };
...
Plogged ($log_file,$modo_ejecucion,1,"- Lanzamos CURL de analisis Alineamiento Planificado");
	...
	$salida_system = system("\"C:\\wget\\wget.exe\" --verbose --user=topercable_runinv --password=T0p3rC4bl3 --no-check-certificate --secure-protocol=TLSv1_2 -c -E -P$directorio_log  http://10.27.47.123:84/Alineamiento_planificado/alineamiento_planificado_run_inventario.asp");
	#$salida_system = 0;
	Plogged ($log_file,$modo_ejecucion,1,"\n\n\n");

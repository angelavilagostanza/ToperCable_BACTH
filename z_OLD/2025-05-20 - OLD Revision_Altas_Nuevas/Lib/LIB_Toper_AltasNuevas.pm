...
sub Get_SF_AltasPortin_Nuevas {
	...
	if ($num_queries > 0) {		
		for (my $i = 0; $i <= $num_queries; $i++) {
			...
			my @query_results = Get_SF_AltasPortin_Nuevas_mock($sqllimit, $sqloffset, $token_sf);
...
sub Get_SF_AltasPortin_Nuevas_mock {

...
sub Get_SF_Posventas {
	...
	Plogged ($log_file,$modo_ejecucion,1,"\t -> $nombre_modulo -> Total registros: $NumRegistros");	
	
	
	# Calcular el numero de consultas necesarias
	if ( $NumRegistros > 0 ) {		
	...
	Plogged ($log_file,$modo_ejecucion,1,"\t -> $nombre_modulo -> Bucles: $num_queries");
	#----------------------------------------------------------------------------------------------------------
...
sub ADD_Alineamiento {
	...
	$sql = "INSERT INTO topercable.alineamiento_planificado (TIPO, NOMBRE, ESTADO, num_msisdn, creador, prioridad) VALUES ('Posventas', Concat(Date_Sub(Curdate(), interval 1 DAY),' - ', DATE_FORMAT(Date_Sub(Curdate(), interval 1 day), '%W'),' -  Bajas (Bajas, PortOut)'),0,'$num_msisdn','ToperCable','3') ";
	$sthd = $dbhd->prepare($sql);	
	$sthd->execute() or die ("No se pudo ejecutar la consulta. SQL:($sql)  Desc(" . $sthd->errstr . ")");

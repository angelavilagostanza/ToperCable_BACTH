...
my $dir_principal			= "D:\\Intranet\\Script\\ToperCable\\Alineamientos_Planificados_Salesforce_Run\\LOGS\\";
...
Plogged ($log_file,$modo_ejecucion,1,"- Creamos el Directorio LOG para la ejecucion del WGet");
	$salida_system = system("mkdir $directorio_log ");	
		if ($salida_system == 0){
			...
			$texto_mail = "<b><font color=red>Mensaje de error del modulo que ejecuta los Alinmeamientos Planificados</b><br><br>No se pudo crear el directorio para los LOGS del envio.</b></font>";
...
Plogged ($log_file,$modo_ejecucion,1,"- Lanzamos CURL de analisis Alineamiento Planificado");
	...
	#$salida_system = system("\"C:\\wget\\wget.exe\" --verbose --user=topecable_runsf --password=T0p3rC4bl3 --no-check-certificate --secure-protocol=TLSv1_2 -c -E -P$directorio_log  http://10.27.47.123:84/Alineamiento_planificado/alineamiento_planificado_run_msa.asp");
	#$salida_system = 0;
	Plogged ($log_file,$modo_ejecucion,1,"\n\n\n");

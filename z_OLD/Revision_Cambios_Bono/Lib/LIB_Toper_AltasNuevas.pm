...
our @ISA    = qw(Exporter);
our @EXPORT = qw( get_sf_token Get_SF_Cambios_BonoSIM ADD_Alineamiento ADD_MSISDN_To_Alineamiento);
...
sub Get_SF_Cambios_BonoSIM {
	...
	if ($num_queries > 0) {		
		for (my $i = 0; $i <= $num_queries; $i++) {
			...
			my @query_results = Get_SF_Cambios_BonoSIM_mock($sqllimit, $sqloffset, $token_sf);
...
sub ADD_Alineamiento {
	...
	$sql = "INSERT INTO topercable.alineamiento_planificado (TIPO, NOMBRE, ESTADO, num_msisdn) VALUES ('Cambios Bono-SIM', Concat(Date_Sub(Curdate(), interval 1 DAY),' - ', DATE_FORMAT(Date_Sub(Curdate(), interval 1 day), '%W'),' -  Cambios bono o SIM'),0,'$num_msisdn') ";	
	$sthd = $dbhd->prepare($sql);	
	$sthd->execute() or die ("No se pudo ejecutar la consulta. SQL:($sql)  Desc(" . $sthd->errstr . ")");
...
sub Get_SF_Cambios_BonoSIM_mock {

...
sub Get_NumErrores_Planificado {
    ...
    Plogged($log_file, $modo_ejecucion, 1, "\t -> $nombre_modulo -> NumErrores: $num_errores");
    return $num_errores;
}
#------------------------------------------------------------------
...
sub UPDATE_Match_Desalineamiento {
    ...
    my $nombre_modulo = (caller(0))[3];
    Plogged($log_file, $modo_ejecucion, 1, "\t -> $nombre_modulo -> planificado_id: $planificado_id | errores: $num_errores");

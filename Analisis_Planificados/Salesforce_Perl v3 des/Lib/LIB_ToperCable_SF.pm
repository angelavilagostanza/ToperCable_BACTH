...
# Get Token para la APi de Salesforce
sub get_sf_token {
	...
	my $request_body = '{"username": "operacionesitn2@masmovil.com", "password": "b*GbgbyS7pD9$4UIlbl^z7uNY"}';
...
sub Get_Planificados_Pendientes {	
	...
	$sql = "SELECT id from topercable.alineamiento_planificado WHERE id = 1193;";
...
sub Get_MSISDN_Planificados ($) {	
	...
	Plogged ($log_file,$modo_ejecucion,0," \t -> $nombre_modulo -> Sacamos los MSISDN pendientes");
	$sql = "SELECT distinct msisdn FROM alineamiento_planificado_detalle WHERE planificado_id = $planificado_id  LIMIT 2000;";
	#Plogged ($log_file,$modo_ejecucion,1," \t -> $nombre_modulo -> SQL: ($sql) ");
	$sthd = $dbhd->prepare($sql);
	$sthd->execute() or die ("No se pudo ejecutar la consulta. SQL:($sql)  Desc(" . $sthd->errstr . ")");
...
sub Batch_SQL_Ejecutar {
    ...
    if ($sql_total ne "") {
		Plogged ($log_file,$modo_ejecucion,1,"\t -> $nombre_modulo *-> Ejecutaado UPDATE SQL.. ");

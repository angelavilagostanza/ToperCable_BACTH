...
my $dir_principal			= "D:\\Intranet\\Script\\ToperCable\\Analisis_Planificados\\Salesforce_descendente\\LOGS\\";
...
Plogged ($log_file,$modo_ejecucion,1,"- Lanzamos CURL de analisis Alineamiento Planificado");	
	...
	$salida_system = system("\"C:\\wget\\wget.exe\" --verbose --user=topecable_runsf --password=T0p3rC4bl3 --no-check-certificate --secure-protocol=TLSv1_2 -c -E -P$directorio_log  http://10.27.47.123:84/Alineamiento_planificado/alineamiento_planificado_run_salesforce_descendente.asp");
	#$salida_system = 0;
	Plogged ($log_file,$modo_ejecucion,1,"\n\n\n");

										...
										if (!$record_promociones->{result}) {
											...
											if ($record_promociones->{SF_Response}->{totalSize} ne "0") {
												...
												for (my $Z = 0; $Z <= $record_promociones->{SF_Response}->{totalSize} - 1; $Z++) {
													...
													my $json_raw = $record_promociones->{SF_Response}->{records}[$Z]{'vlocity_cmt__JSONAttribute__c'};
													my $atributos_promociones;

													# Validamos la obtencion del JSON
													eval {
														$atributos_promociones = decode_json($json_raw);
													};
													if ($@) {
														Plogged($log_file, $modo_ejecucion, 1,"[ERROR] JSON de Tarifa invalido en promocion. index:($Z: $@)");
														$SF_BonosPromociones = "[Error.JSON.Tarifa]";
														next;   # 👉 seguimos con la siguiente iteración del FOR
													}

...
our @ISA    = qw(Exporter);
our @EXPORT = qw( get_sf_token Get_SF_Bajas ADD_Alineamiento ADD_MSISDN_To_Alineamiento);
...
sub ADD_Alineamiento {
	...
	$sql = "INSERT INTO topercable.alineamiento_planificado (TIPO, NOMBRE, ESTADO, num_msisdn, creador, prioridad) VALUES ('Bajas', Concat(Date_Sub(Curdate(), interval 1 DAY),' - ', DATE_FORMAT(Date_Sub(Curdate(), interval 1 day), '%W'),' -  Bajas'),0,'$num_msisdn','ToperCable','3') ";
	$sthd = $dbhd->prepare($sql);	
	$sthd->execute() or die ("No se pudo ejecutar la consulta. SQL:($sql)  Desc(" . $sthd->errstr . ")");
...
sub Get_SF_Bajas {
	...
	#my $sql_query = "SELECT MM_Numero_de_Telefono__c FROM orderitem WHERE LastModifiedDate = YESTERDAY AND MM_Estado_BPM__c = 'Completed' AND Order.Account.RecordType.Name = 'Residencial' AND vlocity_cmt__ParentItemId__c = '' AND MM_Numero_de_Telefono__c != NULL AND DetalleTipo__c Like 'Baja%25' AND vlocity_cmt__AssetId__r.Status != 'Deleted' LIMIT 20000";
	my $sql_query = "SELECT MM_Numero_de_Telefono__c FROM orderitem WHERE LastModifiedDate = YESTERDAY AND MM_Estado_BPM__c = 'Completed' AND Order.Account.RecordType.Name = 'Residencial' AND vlocity_cmt__ParentItemId__c = '' AND MM_Numero_de_Telefono__c != NULL AND Order.MM_changeOrder_type__c = 'Tarifa Desconectar' AND vlocity_cmt__AssetId__r.Status != 'Deleted' LIMIT 20000";

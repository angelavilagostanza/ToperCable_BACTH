...
Plogged ($log_file,$modo_ejecucion,1,"- Obteniendo Bajas de ayer..");
my $resultados  = Get_SF_Bajas();
...
if ($resultados->{result} && $numero_de_registros > 0) {
    ...
    foreach my $registro (@{ $resultados->{SF_Registros} }) {
        if (defined $planificacion_id && $planificacion_id =~ /^\d+$/) {
            Plogged($log_file, $modo_ejecucion, 0, "- Añadiendo MSISDN $registro->{MM_Numero_de_Telefono__c} a la planificacion $planificacion_id ");

...
Plogged ($log_file,$modo_ejecucion,1,"- Obteniendo datos del cliente..");
	...
	if ($result->{result}) {
		...
		Plogged ($log_file,$modo_ejecucion,1,"				
					MSISDN: $result->{SF_msisdn} 
					SF_totalsize: $result->{SF_totalsize} 
					Estado: $result->{SF_Estado}
					Cableoperador: $result->{SF_Cableoperador}
					Residencial: $result->{SF_Residencial}
					CIF: $result->{SF_CIF}
					SF_RootItemId: $result->{SF_RootItemId}
					Marca: $result->{SF_Marca} \n
					Tarifa: $result->{SF_Tarifa}	
					BonoTarifa: $result->{SF_BonoTarifa}		
					BonoCompartido: $result->{SF_BonoCompartido}			
					BonosPromociones: $result->{SF_BonosPromociones}");	
	} else {
		Plogged ($log_file,$modo_ejecucion,1,"- Error al consultar SF: $result->{SF_Response} ");
	}
